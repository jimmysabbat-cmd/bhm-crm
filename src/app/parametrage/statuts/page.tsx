  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-neutral-500">
        Ces statuts apparaissent dans le menu déroulant de chaque dossier. Renomme-les, ajoutes-en,
        ou archive ceux que tu n&apos;utilises plus (ils restent visibles sur les dossiers déjà existants).
      </p>
              ...
              <tr key={s.id} className={`border-b border-neutral-100 last:border-0 ${!s.actif ? "opacity-50" : ""}`}>
                <td className="px-2 py-2">
                  <div className="flex gap-0.5">
                    <form action={async () => { "use server"; await reorder("dossierStatus", s.id, "up"); }}>
                      <button type="submit" disabled={i === 0} className="px-1 text-neutral-400 hover:text-neutral-900 disabled:opacity-30">↑</button>
                    </form>
                    <form action={async () => { "use server"; await reorder("dossierStatus", s.id, "down"); }}>
                ...
                <td className="w-full px-2 py-2">
                  <form action={updateDossierStatus.bind(null, s.id)} className="flex gap-2">
                ...
                <td className="px-2 py-2">
                  <form action={async () => { "use server"; await toggleDossierStatus(s.id, !s.actif); }}>
      ...
      <form action={createDossierStatus} className="flex gap-2">
        <input name="label" placeholder="Nouveau statut..." required className={inputClass} />
