...
export default async function TerminesPage() {
  ...
  const demandes = await getDemandesForDonneurOrdre(ctx, "termines");
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-8 py-10">
      <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Terminés</h1>
      <DemandesList title="Chantiers terminés" demandes={demandes} />
    </div>
  );
}
